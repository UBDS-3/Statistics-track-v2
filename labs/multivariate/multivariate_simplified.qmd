...
Read in data sets that we will be working with.
...
#| out-width: "60%"
# Hint for question 3:
# Fill in the missing parts according to the instructions below and comment out the code
# Load the "pheatmap" library
# Color palette is predefined for you:
mycolors = colorRampPalette(c("#FFD800", "#0056B9"))(100)
# Next, calculate the correlation matrix for the columns in athletes dataset
# using the cor() function and provide it to the pheatmap function to plot
# pheatmap(___, cellwidth = 20, cellheight = 20,
#         color = mycolors, breaks = seq(-1, +1, length.out = length(mycolors)))
```
...
# Hint
# Select only numeric columns from the data frame and use the apply() function
# with appropriate MARGIN to calculate statistics (e.g. mean or standard deviation)
# column-wise
```
...
2. Make a similar plot showing projection lines onto the $y$-axis and show projected points in yellow. What is the standard deviation of the projected points now?
...
#2
# Your code: use the plotting script from the previous plot and modify
# the aes() inputs in geom_point() and geom_segment()
...
#| fig-height: 4
#| out-width: "80%"
#1 Fill in the missing parts in the code and run it
# reg2 = lm(...) # Fit a linear model
...
# p = ggplot(..., aes(x = ..., y = ...)) + geom_point(shape = 1) + coord_fixed()
# newline = p + geom_abline(intercept = ..., slope = ..., col = "#FFD800")
...
# Fill in the missing parts in the code and run it
# ggplot(scaledathletes, aes(x = ..., y = ...)) + geom_point(shape = 1) + 
#   coord_fixed() + 
...
#| echo: !expr params$include_answers
#1 You code 
# Find the way to view the summary of the pca1 object generated above
...
#| out-width: "60%"
#6 Modify the code 
# fviz_pca_biplot(pca1, label = "var", col.ind = ...) 

#7 Your text answer here
#  
```
...
#1 Your code 
# fviz_eig(..., geom = "bar", bar_width = 0.3) + ggtitle("")
...
#4 Your code 
# fviz_pca_ind(...) + ggtitle("") + ylim(c(-2.5, 5.7))
```
