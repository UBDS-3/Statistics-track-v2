---
title: Regression
subtitle: "July 19 - August 2, 2025"
author: Adapted from Rafael Irizarry & Michael Love <br> by Helena L. Crowell & Wolfgang Huber
format:
  html:
    code-fold: false
    code-tools: true
...
# Colinearity and confounding

We use a "classical" data science / ML data set, the Boston housing data, a dataframe with 506 rows and 14 columns.
...
What is going on? The sign of the coefficient for `dis` has flipped. Does distance from where people work increase or decrease the house price? Let's dig into the correlations between the variables.
...
The p value for this test is not "significant".
...
#| fig-height: 3
#| out-width: "40%"
ggplot(FitnessW, aes(x = Before, y = After)) + geom_point() + geom_abline(col = "#000080")
```
...
We use a dataset that compares the friction coefficients on the different legs of a spider. Specifically, we will ask whether more friction comes from a pushing or pulling motion of the leg. The original paper from which the data originate:
...
We included the data in the GitHub repository. As a fallback, we can also download them from [here](https://raw.githubusercontent.com/genomicsclass/dagdata/master/inst/extdata/spider_wolff_gorb_2013.csv).

<script type='text/javascript' src='bug-min.js'></script> <!-- from https://auz.github.io/Bug/ -->
<script type='text/javascript'>
	new SpiderController({'minDelay':100000, 'maxDelay':200000, 'minBugs':1, 'maxBugs':3, 'mouseOver':'die'});
</script>

```{r}
#| label: readcsv
url <- "https://raw.githubusercontent.com/genomicsclass/dagdata/master/inst/extdata/spider_wolff_gorb_2013.csv"
csv <- "spider_wolff_gorb_2013.csv"
if (!file.exists(csv)) download.file(url, csv, quiet=TRUE)
spider <- read.csv(csv, skip = 1)
```

#### Initial visual inspection of the data

Each measurement comes from one of the leg pairs while a leg is either pushing or pulling.
...
We can make a plot summarizing the measurements for each of the eight pairs:

```{r}
#| label: spiderdata
#| fig.cap: "Comparison of friction coefficients of spiders' different leg pairs. The friction coefficient is calculated as the ratio of two forces (see paper Methods), so it is unitless."
#| fig-width: 4
#| fig-height: 2.5
#| out-width: "70%"
#| message: false
library("ggbeeswarm")
ggplot(spider, aes(y = friction, x = leg)) + geom_beeswarm() +
  geom_boxplot(aes(fill = type), outliers = FALSE, col = "#0000ff80", fill = "#00000000") +
...
* The leg pairs towards the back of the spider (L4 being the last) have higher pulling friction.

::: {.callout-caution collapse="true"}
# Heteroskedasticity
Another thing to notice is that the groups have different spread around their average, what we call *within-group variance*. This phenomenon, also called *heteroskedasticity*, can be a problem for the kinds of linear models we  explore below, since we will be assuming that around the group means, the data are distributed identically, meaning the same variance within each group. The consequence of ignoring the different variances for the different groups is that comparisons between those groups with small variances will be overly "conservative" (because the overall estimate of variance is too large), and comparisons between groups with large variances will be overly confident (because the estimate is too small).

Sometimes it is possible to transform your way out of this: if the spread increases with the mean, such that groups with mean values also have larger spread, a possibility is to transform the data with a function such as the `log` or `sqrt`. This looks like it could be useful here, since three of the four push groups (L1, L2, L3) have the smallest friction values and also the smallest spread.
...
These two estimated coefficients are the mean of the pull observations (the first estimated coefficient) and the difference between the means of the two groups (the second coefficient).

::: {.callout-note title="Question" collapse="false"}

Using the subset that includes only data on the L1 leg pair (`spider.sub`),

1. Compute the average friction of "pull" type legs.
2. Compute the difference in average friction between "push" and "pull" type legs.

```{r}
#| echo: !expr params$answers
...
colnames(X)
# the intercept values are all 1
all(X[, "(Intercept)"] == 1)
# in the second column, there are as many 0/1s as pull/push type legs
table(X[, "typepush" ])
table(spider.sub$type)
```

Now we'll make a plot of the `X` matrix by putting black pixels for the 1's and a white pixels for the 0's. This plot will be more interesting for the linear models later on in this script.

- along the $x$-axis are the columns of the design matrix `X`
- along the $y$-axis are the samples (rows of the data)

```{r model_matrix_image, fig.cap="Model matrix for LM with one variable."}
#| code-fold: true
imagemat <- \(x,
    col=c("lavender","blue"),
    las=1, xlab="", ylab="", ...) {
    image(
      ...
      z = t(x), col = col, ...,
      ylim = 0.5 + c(nrow(x), 0),
      las = las, xlab = xlab, ylab = ylab, useRaster = TRUE)
}
imagemat(X, main="Model matrix for LM with one variable")
```
...
Now we'll continue and examine the full dataset, including the observations from all leg pairs. In order to model both the leg pair differences (L1, L2, L3, L4) and the push vs. pull difference, we need to include both terms in the model formula. Let's see what design matrix will be formed with two variables in the formula:
...
Use the equation above to show that the output of `lm()` corresponds to least squares estimates (hint: in R, the transpose can computated with `t()`, matrices are multiplied with `%*%`, and `solve()` gives the inverse of a matrix).
...
1. Compute the average friction for L1-pull and -push legs.
2. Compare these to their corresponding (sum of) coefficient(s).

```{r}
#| echo: !expr params$answers
#| eval: !expr params$answers
s <- split(spider$friction, spider$group)
# the average of L1-pull legs is the intercept
c(mean=mean(s[["L1pull"]]), coef=coefs[[1]])
# the average of L2-push legs is the intercept + the 1st coefficient
c(mean=mean(s[["L1push"]]), coef=sum(coefs[1:2]))
```

:::

In this case, the fitted means for each group, derived from the fitted coefficients, do not line up with those we obtain from simply taking the average from each of the eight possible groups.

The reason is that our model uses five coefficients, instead of eight. We are **assuming** that the effects are additive. However, as we demonstrate in more detail below, this particular dataset is better described with a model including interactions.
...
We can show that the least squares estimates of a linear combination of coefficients is the same linear combination of the estimates. Therefore, the effect size estimate is just the difference between two estimated coefficients.

The contrast vector used by `contrast` is stored as a variable called `X` within the resulting object (not to be confused with our original $\mathbf{X}$, the design matrix):
...
1. the difference in coefficients for L3 and L2, and
2. the linear combination of coefficients (using `cT`)

are the same as `L3vsL2`'s `Contrast` output.
...
L2push.vs.pull <- contrast(fitX,
    ...
    list(leg="L2", type = "pull"))
L2push.vs.pull
# we know this is also orange + yellow
coefs[2] + coefs[6]
```
...
We can't make this contrast using the `contrast` function shown before, but we can make this comparison using the `glht` (for "general linear hypothesis test") function from the *multcomp* package.

::: {.callout-note title="Question" collapse="false"}

1. Create a 1-row matrix `C` that has a -1 for the `typepush:legL2` coefficient and a +1 for the `typepush:legL3` coefficient.
2. Load the `multcomp` package, and use the `glht()` function with `linfct=C` to make this comparison for the `fitX` model from above.
3. Print a `summary()` of estimated interaction coefficients.
4. Subtracting which two coefficients (`coefs`) gives the same estimate?
...
# we know this is also brown - yellow
coefs[7] - coefs[6]
```
...
The first line tells us that adding a variable `type` (push or pull) to the design is very useful (reduces the sum of squared residuals) compared to a model with only an intercept. We can see that it is useful, because this single coefficient reduces the sum of squares by 42.783.

::: {.callout-note title="Question" collapse="false"}

Compute the original sum of squares of the model with just an intercept: $\sum_i(y_i-\hat y)^2$ where $\hat y$ is the average (observed) friction, and $i=1,2,\dots$ enumerates observations; assign your result to a variable called `initial.ss`.
...
Ccompute the sum of squared residuals for the model with only the type information.

1. For each type, compute the sum of squared residuals using the same formula as above.
2. Sum these across types, and assign your result to a variable called `after.type.ss`.

```{r}
#| echo: !expr params$answers
#| eval: !expr params$answers
# split observed 'friction' by 'type'
s <- split(spider$friction, spider$type)
# compute sum of squared residuals for each 'type'
ss.by.type <- sapply(s, function(x) {
    residual <- x - mean(x)
    sum(residual^2)
})
# sum across 'type's
(after.type.ss <- sum(ss.by.type))
```

```{r echo=FALSE}
s <- split(spider$friction, spider$type)
ss.by.type <- sapply(s, \(x) sum((x-mean(x))^2))
after.type.ss <- sum(ss.by.type)
```
