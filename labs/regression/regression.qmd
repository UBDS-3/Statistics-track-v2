...
<script type='text/javascript'>
			new SpiderController({'minBugs':1, 'maxBugs':4, 'mouseOver':'random'});
</script>
...
# all 1s
all(X[, 1] == 1)
# as many 0/1s as pull/push type legs
table(X[, 2]) == table(spider.sub$type)
```

Now we'll make a plot of the $\mathbf{X}$ matrix by putting a black block for the 1's and a white block for the 0's. This plot will be more interesting for the linear models later on in this script.

- along the x-axis is the column of the design matrix $\mathbf{X}$
- along the y-axis is the sample number (the row number of the `data`)
