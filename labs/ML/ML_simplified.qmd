...
```{r}
# Write your code to answer the questions above
...
estimate_mcl_loocv = function(x, grouping) {
  ...
  vapply(seq_len(nrow(x)), function(i) {
    # fit a model with i-th observation left-out
    fit  = lda(x = x[...], grouping = grouping[...])
    # predict on the training data
    ptrn = predict(fit, newdata = x[...,, drop = FALSE])$class
    # predict on the test data i,e, left-out i-th observation
    ptst = predict(fit, newdata = x[...,, drop = FALSE])$class
    # return training and test errors
    c(
      train = mean(ptrn != grouping[...]),
      test = (ptst != grouping[...])
    )
  }, FUN.VALUE = numeric(2)) %>% rowMeans
}
...
```{r}
# Complete the code 
# estimate_mcl_loocv(x = diabetes[, ...], grouping = ...)
# estimate_mcl_loocv(x = diabetes[, ...], grouping = ...)
```
...
# Alternatively, read in zeller ExpressionSet as rds file
# zeller <- readRDS("zeller.rds")
```
...
```{r}
# Hints
# 1. Use the coef() function on glmfit object to get a matrix of coefficients for 
# each lambda (fitted_beta)
# 2. Check the dimensions of the obtained object
# 3. Calculate the L1Norm using colSums() functions on absolute values of coefficients (L1Norm)
# 4. Find out which L1 norm is closest to 6000:
# idx_L1Norm_6k <- which.min(abs(... - 6000))  
# Extract the corresponding lambda
# lambda_6k <- glmfit$lambda[...] 
# lambda_6k
```
...
# 
# # confusion table
# table(predicted = ..., truth = zellerNC$disease)
```
...
# coeffs <- coef(..., s=...)
# # top coefficient
# idx_largest_beta <- which.max(abs(as.numeric(...)))
# rownames(coef(glmfit))[...]
```
