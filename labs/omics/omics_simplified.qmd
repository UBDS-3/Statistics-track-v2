...
# Find out what "Kang18" refers to and write your explanation below:
# "Kang18" is ...
```
...
```{r qc}
# Remove genes (rows) that have zero counts across all ce
...
# Keep only the genes that have at least 1 count in at least 10 cells
...
#     geom_line(aes(y=...), col="blue", lty=2)
```
...
```{r plt-pca-stim, fig.width=5, fig.height=4, echo = F}
...
```{r plt-pca-cell, fig.width=6, fig.height=4, echo = F}
