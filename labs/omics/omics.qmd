...
ggplot(df, aes(x=____, y=_____)) +
    geom_point() +
    geom_point(data=__, col="red") +
    geom_line(aes(y=____), col="blue", lty=2)
...
df <- data.frame(___, colData(___))
ggplot(__, aes(___, ___, col=____)) +
    geom_point(size=0.2) + ___________() +
...
ggplot(__, aes(___, ___, col=log10(___))) +
    geom_point(size=0.2) + coord_equal() +
...
nk <- max(nlevels(__$k), nlevels(__$cell))
ggplot(__, aes(col=_)) +
ggplot(__, aes(col=____)) +
plot_layout(nrow=1) &
